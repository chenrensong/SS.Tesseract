    ...
    public class PixToBitmapConverter
    {
        ...
        private void TransferPalette(Pix pix, Bitmap img)
        {
            ...
            if (colormap != null && colormap.Count <= maxColors)
            {
                ...
                {
                    var color = (System.Drawing.Color)colormap[i];
                    pallete.Entries[i] = System.DrawingCore.Color.FromArgb(color.A, color.R, color.G, color.B);
                }
            }
            else
            {
